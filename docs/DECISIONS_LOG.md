...
### Multi-Language Validation & Tie-Breaking Resolution Semantics
**Decided:**
1. **Same-Domain Batch-Commit Ties are Unresolved by Design:** When a batch commit introduces multiple sibling files in the exact same domain (e.g. 5 `core` JSON codecs in Gin, or 13 `core` AST interfaces in Gson's 2008 initial commit), both historical timestamps ($t_1 == t_2$) and domain priority ranks are co-equal. The tie-breaker intentionally marks these files as `unresolved` (falling back to deterministic secondary sorting or constrained LLM discernment) rather than inventing an arbitrary or false causal order. This behavior is transparently disclosed to users via confidence badges on the resulting build sequence, avoiding false precision.
2. **TypeScript/JS Monorepo Import Resolution Limitation:** Multi-package monorepos (e.g. `colinhacks/zod` with pnpm/npm workspace aliases and `.js` $\leftrightarrow$ `.ts` ESM imports) currently produce mostly unlinked/isolated files because `JavaScriptLanguageParser` treats workspace package identifiers as external dependencies. Build-sequence reconstruction on TypeScript monorepos is formally recorded as a known parser gap deferred to v2.
